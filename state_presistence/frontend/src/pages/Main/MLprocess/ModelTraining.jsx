...
export default function ModelTraining({ config, onPrevStep, onNextStep }) {
  ...
  const handleTrain = async () => {
    try {
      ...
      setError('');
      setResult(null);
      let endpoint = "/home/mlprocess/regression";
      if (config.task_type?.toLowerCase() === "classification") {
        endpoint = "/home/mlprocess/classification";
      } else if (config.task_type?.toLowerCase() === "clustering") {
        endpoint = "/home/mlprocess/clustering";
      } else if (config.task_type?.toLowerCase() === "dimensionality reduction") {
        endpoint = "/home/mlprocess/dimensionality_reduction";
      }
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 shadow-sm relative">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            {config.task_type?.toLowerCase() !== "clustering" && config.task_type?.toLowerCase() !== "dimensionality reduction" && (
              <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
            ...
            {config.task_type?.toLowerCase() !== "clustering" && config.task_type?.toLowerCase() !== "dimensionality reduction" && (
              <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                ...
                <span className="text-sm font-bold text-slate-800">{Math.round((config.test_size || 0) * 100)}%</span>
              </div>
            )}
          </div>
                ...
                <>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-emerald-100 hover:border-emerald-300 transition-colors">
                    <span className="block text-sm font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                      Silhouette Score
                      <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full normal-case">Higher is better (Closer to 1)</span>
                    </span>
                    <span className="text-4xl font-black text-slate-800 tracking-tight">{result.silhouette_score.toFixed(4)}</span>
                  </div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-emerald-100 hover:border-emerald-300 transition-colors">
                    <span className="block text-sm font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                    ...
                    <span className="text-4xl font-black text-slate-800 tracking-tight">{result.davies_bouldin_score.toFixed(4)}</span>
                  </div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-emerald-100 hover:border-emerald-300 transition-colors sm:col-span-2">
                    <span className="block text-sm font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                      Number of Clusters
                    </span>
                    <span className="text-4xl font-black text-slate-800 tracking-tight">{result.n_clusters}</span>
                  </div>
                </>
              ) : result.reduced_features !== undefined ? (
                <>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-emerald-100 hover:border-emerald-300 transition-colors">
                    <span className="block text-sm font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                      Original Features
                    </span>
                    <span className="text-4xl font-black text-slate-800 tracking-tight">{result.original_features}</span>
                  </div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-emerald-100 hover:border-emerald-300 transition-colors">
                    <span className="block text-sm font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                      Reduced Features
                    </span>
                    <span className="text-4xl font-black text-slate-800 tracking-tight">{result.reduced_features}</span>
                  </div>
                  {result.explained_variance_sum !== null && result.explained_variance_sum !== undefined && (
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-emerald-100 hover:border-emerald-300 transition-colors sm:col-span-2">
                      <span className="block text-sm font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                        Explained Variance Ratio (Sum)
                        <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full normal-case">Higher is better (Closer to 1.0)</span>
                      </span>
                      <span className="text-4xl font-black text-slate-800 tracking-tight">{(result.explained_variance_sum * 100).toFixed(2)}%</span>
                    </div>
                  )}
                </>
